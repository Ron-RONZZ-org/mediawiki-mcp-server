...
- `image-search` — searches Wikimedia Commons (public API, no account) for images and returns, per match, the file page and direct file URLs, a thumbnail URL, dimensions and MIME type, and the license, author and credit (attribution requirements). Pairs with `upload-file-from-url` to land a chosen image on a wiki with its attribution. Filterable to CC0/public-domain results (`license='no_restrictions'`).
- Six tools for wikis running the [EmbeddableContent](https://www.mediawiki.org/wiki/Extension:EmbeddableContent) and [WikibaseCitation](https://www.mediawiki.org/wiki/Extension:WikibaseCitation) extensions, mirroring the extensions' `Special:Add*` forms and render APIs field-for-field:
  ...
  - `embeddable-add-semantic-entity` creates or updates a person, software (FOSS), collective, fictional-character or other-class item, with the fields of the Special:AddPerson / AddSoftware / AddCollective / AddFictionalCharacter forms: derived labels (given/family name, with the "(fictional character)" suffix), birth/death dates and places at day precision, ORCID/VIAF/ISNI/Wikidata/OpenAlex author IDs, official website, developer/license/operating-system/user-interface/has-use item IDs, programming language (resolved from a label), repository and documentation URLs, the collective-class picker presets (or a direct class item ID), parent organization, present-in-work items with the auto-generated "fictional character in …" description, and — for kind=other — instanceOf plus raw statements.
...
- `embeddable-add-citation-source` now delegates to the wiki's own AddSource service (`action=addsource`): the field contract, validation, statement building and the classic `Source:` page + sitelink all run on the wiki, so the tool can never drift from the Special:AddSource form again. The created item's classic page and page↔item sitelink are now also created (previously the tool wrote the item only). Requires an EmbeddableContent version that ships the module.
- `embeddable-describe-entity-type` serves the citation-source section from the wiki's `action=addsource-fields` endpoint — the field contract has one publisher on the wiki.
- `embeddable-add-special-content` now delegates to the wiki's own special-content service (`action=addspecialcontent`): payload handling (math delimiter stripping, multi-line backslash-escaping), field validation and statement building all run on the wiki, so the tool can never drift from the AddQuotation / AddMath / AddCodeSnippet forms again. The tool still resolves a programming-language label to its item id locally before calling. Quotation attribution is now enforced on create (as documented). The describe tool's special-content section comes from `action=addspecialcontent-fields`.
- `embeddable-add-semantic-entity` now delegates to the wiki's own semantic-entity service (`action=addsemanticentity`): label derivation (person given/family, fictional-character suffix + auto-description), collectiveClass preset resolution, field validation and statement building all run on the wiki, and the classic `Person:` / `Collective:` / `FOSS:` page + sitelink are now created like the forms. Raw statements for kind=other are no longer accepted — use `wikibase-edit-entity` for those. The describe tool's semantic-entity section comes from `action=addsemanticentity-fields`; the MCP-side field tables (CLASS_FIELDS etc.) are deleted — the wiki is the single source of the field contract.
- `embeddable-add-citation-source`, `embeddable-add-semantic-entity` and `embeddable-add-special-content` no longer collapse a create the wiki's duplication guard refuses into a generic upstream failure. The guard's answer is returned as a result: the item was **not** created, and `duplicateOf` names the existing item the record collides with (an identical authority id or URL, or a highly similar class-filtered label) — cite or update that item instead. Each tool also accepts `confirmDuplicate` to force the create past the guard, mirroring the API modules' machine-client contract.
- The same three add tools now resolve a write whose response is lost mid-flight (no result, no error). A create is checked against the term store by the label it would have stored — an item with that label exists → reported as likely-created with its id; none exists → reported as not created, so retrying is safe. A lost update response states that re-running it produces the same statements (fields provided are replaced, blank fields keep the current ones). Previously the tools reported "the wiki accepted the request but returned no … result", which did not distinguish a landed create from a no-op.
- `wikibase-edit-entity`, `wikibase-add-statement` and `wikibase-setsitelink` now resolve a write whose response is lost mid-flight the same way, each by re-reading the wiki: `wikibase-setsitelink` checks the item's sitelinks and mirrors the success payload when the link landed or reports a definitive not-set (retry-safe) otherwise; `wikibase-add-statement` checks the entity's claims for the property (item values compared by entity id, text datatypes by literal) and reports the matched statement id or a definitive not-added (retry-safe); `wikibase-edit-entity` checks a term-only update against the entity's current terms (state matches → success payload, differs → retry-safe), verifies a lost create by a term-store label search (found → likely-created — never declared retry-safe, since creating has no duplication guard), and gives targeted guidance for updates carrying claims or sitelinks (verify with `wikibase-get-entity`, re-run only the absent parts — a blind re-run of a landed update adds GUID-less statements again) and for `clear=true` (re-running is safe — the entity is replaced with exactly the data). Previously all three reported "the wiki accepted the request but returned no entity/statement" plus the raw response.
- `search-page`'s description now routes Wikibase entity lookups to `wikibase-search-entities` / `wikibase-get-entity` / `wikibase-query`: full-text page search covers wiki-page text, which a stock search backend does not index for entity terms, so a term that exists only as an item or property label returned no matches and left callers unsure whether the entity existed.
